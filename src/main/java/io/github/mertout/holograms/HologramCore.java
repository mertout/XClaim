...
public class HologramCore {
    ...
    public void createHologram(@NotNull final Location loc, @NotNull final DataHandler data) {
        ...
        loc2.setY(loc2.getY() + HologramFile.get().getDouble("settings.hologram-height"));
        loc2.setZ(loc2.getZ() + 0.5);
        if (Bukkit.getPluginManager().getPlugin("DecentHolograms") != null) {
            if (DHAPI.getHologram(data.getOwner().toString()) != null) {
                DHAPI.getHologram(data.getOwner().toString()).destroy();
            }
            eu.decentsoftware.holograms.api.holograms.Hologram holo = DHAPI.createHologram(data.getOwner().toString(), loc2);
            for (String str : HologramFile.get().getStringList("settings.hologram-lines")) {
                DHAPI.addHologramLine(holo, HexColor.hex(str
                        .replace("{owner}", data.getOwner().toString())
            ...
            Hologram holo = HologramsAPI.createHologram(Claim.getInstance(), loc2);
            for (String str : HologramFile.get().getStringList("settings.hologram-lines")) {
                holo.appendTextLine(HexColor.hex(str
                        .replace("{owner}", data.getOwner().toString())
                        .replace("{remaining-time}", ClaimManager.calcTime(data))
                        .replace("{team-size}", data.getMembers().size() + "")
                        .replace("{creation-date}", data.getCreationDate())));
    ...
    public void moveHologram(@NotNull final DataHandler data, Location loc) {
        ...
        if (Bukkit.getPluginManager().getPlugin("DecentHolograms") != null) {
            loc2.setX(loc2.getX() + 0.5);
            loc2.setY(loc2.getY() + HologramFile.get().getInt("settings.hologram-height"));
            loc2.setZ(loc2.getZ() + 0.5);
        ...
        for (final DataHandler data : Claim.getInstance().getClaims()) {
            ...
            if (Bukkit.getServer().getOnlinePlayers().size() > 0) {
                if (Bukkit.getPluginManager().getPlugin("DecentHolograms") != null) {
                    ...
                    int index = 0;
                    for (String str : HologramFile.get().getStringList("settings.hologram-lines")) {
                        DHAPI.setHologramLine(holo, index, HexColor.hex(str
